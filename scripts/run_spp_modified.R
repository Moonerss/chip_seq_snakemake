...
# ===================================	
	...
	cat('-samtoolsPath=<samtoolsPath>, the Path of samtools\n',file=stderr())
...
# ===================================	
	...
	for (each.arg in args) {
		...
		} else if (grepl('^-samtoolsPath=',each.arg)) {
			arg.split <- strsplit(each.arg,'=',fixed=TRUE)[[1]] # split on =
			if (! is.na(arg.split[2]) ) {
				samtoolsPath <- arg.split[2] # second part is temp.dir
			} else {
				stop('No samtools Path provided for parameter -samtoolsPath=')
			}
...
read.align <- function(align.filename, samtoolsPath) {
# ===================================
...
# ===================================	
	if (grepl('(\\.bam)?.*(\\.tagAlign)',align.filename)) { # if tagalign file
		...
		command[1] <- sprintf("%s view -F 0x0204 -o - %s", samtoolsPath, align.filename)
...
chip.data <- read.align(ta.chip.filename, iparams$samtoolsPath)
...
if (! is.na(iparams$control.file)) {
	...
	control.data <- read.align(ta.control.filename, iparams$samtoolsPath)
